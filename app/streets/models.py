...
class SurfaceType(models.Model):
    ...
    rating = models.PositiveSmallIntegerField(blank=False, null=True, 
                                                help_text = 'Чем выше оценка, тем удобнее ездить по данному покрытиию', verbose_name='Оценка', 
                                                validators=[MinValueValidator(1), MaxValueValidator(10)], choices= RATING_COUNT_CHOICES, default=1)
    def __str__(self):
        return self.name
...
class Street(models.Model):
    street_type = models.ForeignKey(StreetType, blank=True, null=True, on_delete=models.CASCADE, verbose_name='Тип улицы',
                                  related_name='street_type')
    street_surface = models.ForeignKey(SurfaceType, blank=True, null=True, on_delete=models.CASCADE, verbose_name='Тип покрытия',
